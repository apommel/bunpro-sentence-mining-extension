...
const BUNPRO_COOKIE_URL = "https://bunpro.jp";
const BUNPRO_COOKIE_NAME = "frontend_api_token";
...
async function getSettings() {
  return new Promise((resolve) => {
    api.storage.sync.get(["llmBaseUrl", "llmApiKey", "llmModel", "openVocabTab"], resolve);
  });
}
...
async function llmProcessSentence(selectedWord, fullSentence, settings) {
  ...
  {
    role: "system",
    content: `You are an expert Japanese linguist and educational content creator. Your task is to generate structured JSON data for Japanese learning flashcards based on a provided "Selected word" and "Full sentence".
...
{"dictionary_form":"不満","content":"先生（せんせい）に対（たい）する____を口（くち）にする。","answer":"不満（ふまん）","translation":"<p>To express <strong>dissatisfaction</strong> towards the teacher.</p>"}`
    ...
    {
      role: "user", 
      content: `Selected word: ${selectedWord}\nFull sentence: ${fullSentence}`
    }
  ];
